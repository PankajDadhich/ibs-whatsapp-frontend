...
const CreatePlan = () => {
  ...
  const handleChange = (e) => {
    ...
    if (name.startsWith("plan_info")) {
      ...
      if (
        (field === "number_of_whatsapp_setting" ||
  ...
  const handleFormSubmit = async (e) => {
    ...
    try {
      const result = payload.plan_info.id
        ? await WhatsAppAPI.updatePlanRecord(payload)
        : await WhatsAppAPI.insertPlanRecord(payload);

      if (result.success) {
        toast.success(
          payload.plan_info.id
                          ...
                          <Col lg={6} sm={12} xs={12}>
                            <Form.Group className="mb-3">
                              ...
                              <Form.Control
                                ...
                                value={
                                  formData.plan_info.number_of_whatsapp_setting
                                }
                                onChange={handleChange}
                                required
                              />
                            ...
                            <tbody>
                              {modules &&
                                modules.map((module, idx) => (
                                  <tr key={idx}>
                                    <td>
                                      <i className={`${module.icon} mx-1`}></i>{" "}
                                      {module.name}
                                    </td>
                                    <td>
                                      <Form.Check
