...
const MessageTemplateModal = ({ show, onHide, contactData, refreshData, filterData,selectedWhatsAppSetting }) => {
    ...
    const handleCategoryChange = (e) => {
        ...
        const filtered = allTemplateData.filter(template => template.category === selectedCategory);
    ...
    const handleSubmit = async (event) => {
        ...
        if (parameters.sendToAdmin && userInfo?.phone) {
            const formattedPhone = userInfo.phone.match(/^\d{10}$/) ? `91${userInfo.phone}` : userInfo.phone;
        ...
        try {
            ...
            for (const contact of body) {
                ...
                const result = isTextMessage
                    ? await WhatsAppAPI.sendSingleWhatsAppTextMessage(singleText,selectedWhatsAppSetting)
                    : await WhatsAppAPI.sendWhatsAppTemplateMessage(singleText,selectedWhatsAppSetting);
                if (result.error) {
    ...
    return (
        <>
            ...
            <Form.Group className='mb-4'>
                <Form.Label className="form-view-label" htmlFor="formBasicCategory">
