...
const TemplateModal = ({
    ...
    try {
          ...
          components: [
            {
              ...
              parameters: documentId
                ? [
                    {
                      type: selectedTemplateName.header.toLowerCase(),
                      [selectedTemplateName.header.toLowerCase()]: {
                        id: documentId,
                      },
                    },
                  ]
              ...
              parameters:
                selectedTemplateName.category !== "AUTHENTICATION"
                  ? Object.keys(parameters)
                      .filter((key) =>
                        selectedTemplateName.message_body.includes(`{{${key}}}`)
                      )
                      .map((key) => ({ type: "text", text: parameters[key] }))
                  : [],
            },
          ],
        },
      ...
      if (
        ...
        reqBody.template.components.push({
          ...
          index: 0,
          parameters: [
            { type: "text", text: selectedTemplateName.example_body_text },
          ],
        });
      }

      // Step 4: Send Message
      const result = await WhatsAppAPI.sendWhatsAppTemplateMessage(
        reqBody,
        selectedWhatsAppSetting
      );
      ...
      if (parameters.sendToAdmin && userInfo?.phone) {
        let adminPhone = userInfo.phone.match(/^\d{10}$/)
          ? `91${userInfo.phone}`
          : userInfo.phone;
        const adminReqBody = { ...reqBody, to: adminPhone };
        const adminResult = await WhatsAppAPI.sendWhatsAppTemplateMessage(
          adminReqBody,
          selectedWhatsAppSetting
        );
  ...
  //   console.log(
  //     "uu",
  //     selectedTemplateName?.message_body?.match(/\{\{(\d+)\}\}/g)
  //   );
  //   console.log(parameters);
  ...
  //   const filteredValues = Object.keys(filteredParams);

  //   const hasMatch = filteredValues.some((value) =>
  //     selectedTemplateName?.message_body
  //       ?.match(/\{\{(\d+)\}\}/g)
  //       .includes(`{{${value}}}`)
  //   );

  //   console.log("Matching Found:", hasMatch); // true or false
