...
const Payment = (props) => {
  ...
  const init = async (id, action) => {
    ...
    if (action === "pay") {
      ...
      if (response[0].invoice.plan_id) {
        ...
        const plan = await WhatsAppAPI.getPlansById(
          response[0].invoice.plan_id
        );
        if (plan) {
          setPlanById(plan);
        }
      ...
      if (response) {
        ...
        setInvoice((prev) => ({
          ...prev,
          user_id: response.companyResult[0].user_id,
        }));
      } else {
        setCompany({});
      }
  ...
  const handleChange = async (e) => {
    if (e.target.name === "user") {
      ...
      setInvoice({
        ...
        validity: e.target.getAttribute("data-validity"),
      });
    } else if (e.target.name === "plan") {
      const planid = e.target.value;
      let planname =
        e.target[e.target.selectedIndex].getAttribute("data-planname");
      if (planid) {
      ...
      if (e.target.name === "payment_method") {
        if (e.target.value === "upi" || e.target.value === "cheque") {
          document.getElementById("tr_cq_div").style.display = "block";
          document
            .getElementById("transaction_cheque_no")
            .setAttribute("required", true);
        } else {
  ...
  const handleSubmit = async (updatedInvoice) => {
    ...
    if (updatedInvoice?.id) {
      const response = await WhatsAppAPI.updateInvoiceAddTransaction(
        updatedInvoice
      );
      if (response.success) {
      ...
      const response = await WhatsAppAPI.addInvoiceWithTransaction(
        updatedInvoice
      );
      if (response.success) {
  ...
  const validateForm = () => {
    ...
    if (!selectOption.amount) {
      ...
      setRadioError((prev) => ({
        ...prev,
        amount: "Please select an option.",
      }));
    }
              ...
              <Form noValidate>
                <Row className="pb-4 mx-5 mt-5 border-bottom">
                  ...
                  <Col lg={6}>
                    ...
                    <div className="d-flex flex-direction-column">
                      <div className="d-flex align-items-baseline">
                        ...
                        <div>
                          ...
                          <p className="fs-6">{company?.email}</p>
                          <p className="fs-5 mb-0 fw-bold">Contact:</p>
                          <p className="fs-6">{company?.phone}</p>
                        </div>
                      </div>
                      <div className="d-flex align-items-baseline mt-2">
                        ...
                        <Form.Control
                          type="text"
                          id={"another-name"}
                          placeholder="Another Users Name"
                          name={"new_name"}
                          style={{ width: "250px" }}
                          onChange={handleChange}
                          disabled
                    ...
                    {radioError.user && (
                      <p className="fw-bold fs-6 mt-3" style={{ color: "red" }}>
                        {radioError.user}
                      </p>
                    )}
                  </Col>
                ...
                <Row className="pb-4 mx-5 mt-5">
                  ...
                  <Col lg={4}>
                    <Form.Group>
                      <Form.Label
                        className="form-view-label fw-bold fs-6"
                        htmlFor="plans"
                      >
                        Plans
                      </Form.Label>
                      ...
                      >
                        ...
                        {plans?.map((val, index) => (
                          <option
                            value={val.plan_info.id}
                            key={index}
                            data-planname={val.plan_info.name}
                          >
                            {val.plan_info.name}
                          </option>
                        ))}
                  ...
                  <Col lg={4}>
                    <p className="fs-6 fw-bold border-0 mt-3">
                      Number of Users: {planById?.number_of_users ?? "0"}
                    </p>
                    <p className="fs-6 fw-bold border-0 mt-2">
                      Number of Whatsapp Setting:{" "}
                      {planById?.number_of_whatsapp_setting ?? "0"}
                    </p>
                  </Col>
                  <Col lg={4}>
                    ...
                    <div className="mt-1 d-flex flex-direction-column">
                      <div className="d-flex flex-direction-row mb-3">
                        <input
                          ...
                          value={planById?.pricepermonth}
                          onChange={handleChange}
                          checked={
                            invoice?.amount &&
                            Math.floor(invoice?.amount) ===
                              planById?.pricepermonth
                              ? true
                              : false
                          }
                        />{" "}
                        <span className="border-0">
                          {planById?.pricepermonth ?? "--"} per Month
                        </span>
                      </div>
                      <div className="d-flex flex-direction-row ">
                        <input
                          ...
                          data-validity="12"
                          style={{ marginRight: "10px" }}
                          value={planById?.priceperyear}
                          onChange={handleChange}
                          checked={
                            invoice?.amount &&
                            Math.floor(invoice?.amount) ===
                              planById?.priceperyear
                              ? true
                              : false
                          }
                        />{" "}
                        <span className="border-0">
                          {planById?.priceperyear ?? "--"} per Year
                        </span>
                      </div>
                    </div>
                    {radioError.amount && (
                  ...
                  <Col lg={4} id="tr_cq_div" style={{ display: "none" }}>
                    <Form.Group>
                      <Form.Label
                        className="form-view-label fw-bold fs-6"
                        htmlFor="transaction_cheque_no"
                      >
                        Transaction No./Cheque No.
                      </Form.Label>
                      <Form.Control
                        id="transaction_cheque_no"
                        type="text"
                        name="transaction_cheque_no"
                        onChange={handleChange}
                        value={
                          invoice?.transaction_cheque_no !== ""
                            ? invoice?.transaction_cheque_no
                            : ""
                        }
                        style={{ width: "75%" }}
